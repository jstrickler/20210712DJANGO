from django.http import HttpResponse # only used in class (see comment below)
from django.shortcuts import render

# Create your views here.

def home(request):
    return HttpResponse("Welcome to the Oreo App")

def dubl(reqeust):
    return HttpResponse("Double Stuff! Yay! Yay!")

# example without template (only used in class -- always use templates in real life):
# def home(request):
#     return HttpResponse("Welcome to Oreos and their friends")

# example with template (normal Django approach)
# def home(request):
#     context = { 'message': "Welcome to Oreos and their friends" }
#     return render(request, 'oreos/home.html', context)
